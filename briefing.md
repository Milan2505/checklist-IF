...
## CE QUI CHANGE DEPUIS LA REV 22

| # | Objet | Nature |
|---|---|---|
| 1 | **Deux nouvelles cellules lues : `Porte départ` et `Porte arrivée`.** Inscrites au registre du parseur **avant** d'être écrites dans le dossier (§A.3 règle 10), elles rejoignent le cartouche ②, qui se renomme **« Pistes, portes et dégagement »** et passe de 6 à 8 cellules | ajout |
| 2 | **Le total passe de 34 à 36 cellules** estimées au temps 1. Tous les compteurs sont repassés : §A.3 règle 8, §A.4 (trois fois), §A.5 contrôle avant envoi, §8. Aucun `34` de compteur ne survit | correctif |
| 3 | **§A.5 · septième règle d'écriture — `NIL`.** Une valeur qui ne peut pas être estimée s'écrit `NIL`, valeur nue, jamais `[MANQUE]` : ce marqueur rend la cellule illisible et la cinquième ligne de contrôle la rejette. Le motif va en prose. **Règle générale, pas une exception aux portes** — le prochain champ non estimable trouvera la réponse déjà écrite | ajout |
| 4 | Le conflit entre cette règle et la **règle 8 du §A.3** est tranché des deux côtés : une case en `NIL` n'est pas une case à remplir, elle est remplie, et ce qu'elle dit est vrai | correctif |
| 5 | §A.4 — deux méthodes d'estimation ajoutées, et une contrainte : **une porte ne s'estime pas.** C'est une donnée observée sur le traceur de trafic `[RDR]`, ou `NIL`. Aucune déduction depuis le terminal, la compagnie ou l'habitude d'une ligne | ajout |
| 6 | **Au temps 2, les deux portes restent `[RDR]` ou `NIL`** : l'OFP ne les porte pas, aucune ne devient `[OFP]`. La ligne d'origine du cartouche ② le dit explicitement | mise à jour |
| 7 | ⚠ **Une porte réelle et une place de parking dans Infinite Flight sont deux choses différentes.** La porte est une référence de réalisme, pas une contrainte de spawn : elle n'est **pas** un point de vérification du §8 | ajout |
| 8 | §A.5 vocabulaire — la ligne du cartouche ② devient `Piste départ · Cap départ · Porte départ · Piste arrivée · Porte arrivée`. La table « en attente d'inscription au registre » ne bouge pas : les portes en sortent d'emblée, puisqu'elles y sont inscrites avant d'être écrites | mise à jour |
| 9 | **Page · panneau d'identité ouvrable au clic.** Les deux blocs aéroport — départ et arrivée — ouvrent chacun un panneau superposé portant Type, Compagnie et la porte du terrain concerné. Deux panneaux distincts, jamais des onglets ; fermeture au clic extérieur, au bouton ou par Échap ; un seul ouvert à la fois | ajout |
| 10 | Page · le libellé `Compagnie aérienne` porte désormais la mention **code OACI**, là où la valeur se tape et là où elle s'affiche. Le dossier le disait déjà (§A.5, « jamais le nom commercial ») ; la page le dit maintenant aussi | mise à jour |
| 11 | **§0 et son ordre gelé : intacts.** Cette révision ajoute deux cellules et un affichage, elle ne déplace rien — la structure du fichier reste organisée en cartouches, la page groupe par aéroport, et les deux ne se confondent pas | inchangé |
| 12 | **§A.3 règle 10 — « au registre » devient « au registre ET à la liste des champs ».** L'inscription des portes l'a montré : une clé déclarée au registre mais absente de la liste des champs est lue, puis jetée, sans qu'aucun bandeau ne le dise. La règle au singulier laissait passer exactement le défaut silencieux qu'elle prétend empêcher | correctif |
