...
## CE QUI CHANGE DEPUIS LA REV 20

| # | Objet | Nature |
|---|---|---|
| 1 | **§7.0 · Séquence de mise en route (A321neo)** — les sept points proposés de la REV 20 deviennent une **séquence ordonnée en dix phases**, un tableau exécutable du cold & dark au lâcher des freins, avec la condition de passage de chaque phase | ajout |
| 2 | **§7.0 cesse d'être dormante pour l'ORDRE de la procédure.** Elle reste `[WIP]` bouton par bouton : l'ordre vient de l'Airbus réel et ne dépend d'aucun build, le comportement de chaque bouton n'a été vu par personne | levée partielle |
| 3 | Nouveau marqueur **`[RÉEL]`** — procédure et valeurs de référence de l'A320/A321 réel, ni simulateur ni calcul. Il donne l'ordre et l'ordre de grandeur, jamais ce qu'affiche Infinite Flight | ajout |
| 4 | §8 points ouverts — la ligne unique « Section §7.0 » se scinde en **cinq points nommés**, chacun avec son observation de fermeture, tous **non bloquants** tant que l'A321neo n'est pas au sélecteur | correctif |
| 5 | §9 après vol — le bloc SYSTÈMES gagne les trois relevés qui ferment ces points : emplacement de la commande ADIRS, positions réelles de `NORM`, rangée basse de chaque panneau | mise à jour |
| 6 | **§A.4 inchangé**, à l'octet près. §7.0 n'introduit aucun libellé lu par le parseur : son tableau est à six colonnes, donc ignoré à la lecture machine, et aucune de ses valeurs ne touche un cartouche | inchangé |
| 7 | **§A.5 · contrôle avant envoi passe à cinq lignes** — aucune cellule ne porte de marqueur hors des six retirés à la lecture. Le parseur applique désormais la règle et **rejette la cellule en nommant le marqueur** : `[WIP]`, `[MANQUE]` et `[RÉEL]` n'étaient tenus hors des cartouches que par une convention rédactionnelle, et un marqueur qui dérivait cassait une valeur avec un motif qui ne disait pas pourquoi | correctif |
| 8 | §7.0 — le motoriste **sort des points ouverts** : les deux motorisations sont annoncées et se gèrent par livrée, comme l'A330. Un modèle moteur unique porte les deux nacelles, donc les mêmes repères de démarrage | correctif |
...
**FIN · REV 21 · Simulation uniquement**
