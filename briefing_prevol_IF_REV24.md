...
## CE QUI CHANGE DEPUIS LA REV 23

Cette révision **refusionne la branche REV 22 restée de côté**. Deux fichiers portaient ce numéro : celui qui a donné la REV 23 (renumérotation du §0) et un second, jamais intégré, qui portait la matière studio du 29/08/2026. La note « Filiation » de ce second fichier annonçait la refusion : elle est faite ici. Aucune cellule de cartouche ne bouge.

| # | Objet | Nature |
|---|---|---|
| 1 | **§6.7 · APPROCHE AUTOMATIQUE — section dormante.** FMA `CAT3 DUAL` / `AP1+2` relevé sur capture studio du 29/08. Numérotée 6.7 et non 6.4 : la stabilisation occupe déjà le 6.4. Le §6 va désormais jusqu'à **6.7** | ajout |
| 2 | **§7.0 bis · MENU MISC.** Vingt entrées relevées sur capture studio, et ce que trois d'entre elles font au dossier : `AUTO START` court-circuite le §7.0, `CHECKLIST` menace de le rendre inutile, `TIME x1` fausse tout relevé du §9 | ajout |
| 3 | **§7.0 · séquence — trois lignes ajoutées** : `2.2` contrôle PFD/ND, `2.3` accès au FMGC depuis `MCDU MENU`, `9.5` mémos sol et `FOB` avant repoussage. Le tableau passe de 42 à **45 lignes** | mise à jour |
| 4 | **§7.0 · compte des sources corrigé.** Le chiffre de « 30 lignes `[WIP]` » était faux depuis la REV 21 et l'est resté en REV 23 : le tableau en porte **32** sur 45. Corrigé ici et au §8 | correctif |
| 5 | **§7.0 · ce que le live du 28/08 a fermé** — un tableau, et la réponse est *rien*. Aucun panneau Systems dans les captures : les cinq points d'inventaire restent ouverts. Ce que le live a ouvert à la place est ailleurs (§6.7, §7.0 bis, §2.3, ADIRS) | correctif |
| 6 | §7 · ALIGNEMENT ADIRS — un mémo `IR IN ALIGN 2 MN` existe désormais à l'E/WD. La phrase de la REV 21 est corrigée sans être retirée : elle reste vraie tant que le mémo n'a pas été vu en session | correctif |
| 7 | §7 · **contrôle `GW − FOB` au poste** — masse sans carburant lue au poste, comparable directement à l'EZFW de l'OFP sans passer par l'écran Poids et centrage. L'écart de masse du §2.3 se voit maintenant **avant** le repoussage | ajout |
| 8 | §7 · comportements normaux — `IR IN ALIGN`, `EXT PWR` et `DOORS OPEN` rejoignent la liste des mémos sol à ne pas prendre pour des pannes | mise à jour |
| 9 | §2.3 — la page racine `MCDU MENU` porte quatre entrées ; le FMGC n'est plus la page d'ouverture, il se sélectionne | mise à jour |
| 10 | §3 et §9 — un vol conduit en accélération temporelle (§7.0 bis) ne produit aucun relevé exploitable. Le §9 se remplit sur un vol en temps réel, ou pas du tout | ajout |
| 11 | §8 — quatre points ouverts ajoutés, dont **trois se ferment sans voler** : une mise sous tension au parking suffit. Le §6.7 est déclaré non bloquant, comme le §7.0 | ajout |
| 12 | **§A.4 et §A.5 inchangés à l'octet.** Aucun libellé ajouté, aucune cellule ajoutée : **36 cellules, cinq cartouches**, les deux portes de la REV 23 comprises. Rien de ce qui précède n'entre dans un cartouche | note |
...
⚠ **Une entrée `CHECKLIST` figure au menu MISC des captures studio (§7.0 bis).** Si elle porte une checklist cold & dark, la troisième liste ci-dessus perd sa dernière ligne et le §7.0 se réduit à ce que la checklist embarquée ne dit pas. La règle ne s'applique pas sur la foi d'un bouton : elle attend que son contenu soit lu.
...
|---|---|---|
| `AUTO START` | court-circuite toute la séquence du §7.0 d'un appui | **À ne pas utiliser avant que la séquence manuelle ait été tenue une fois de bout en bout.** Sinon les lignes `[WIP]` ne passeront jamais à `[SIM]` : elles resteront invérifiées parce qu'aucune n'aura été touchée |
| `CHECKLIST` | checklists embarquées, périmètre inconnu | Point ouvert du §8. Si elles couvrent le cold & dark, la règle de non-répétition s'applique et le §7.0 se réduit à ce qu'elles ne disent pas. Le bouton ne suffit pas : son contenu se lit avant de retirer quoi que ce soit |
...
`ECAM PAGE` sélectionne une page SD directement — les attendus SD du §7.0 restent valables, le chemin change. `AUTO BRAKES` double la commande d'autobrake du build 18266 (§4). `BRK FAN`, `SNAPSHOTS`, `CAMERA SNAPSHOT`, `MANAGE ORGS`, `ATC SCHEDULE`, `SAVE`, `AUTO PAUSE` et `SCENARIO EDITOR` sont hors périmètre du briefing.
...
OFP de moins de 2 h · MIN DIV et BINGO calculés · distance MCDU = distance OFP · briefing panne fait · schedule volets sorti pour le type · **36 cellules des cartouches renseignées** · **neuf champs du §0.2 bis renseignés, aucun laissé à sa valeur de démonstration** · type et masse à vide donnés par Milan, aucune alerte du §A.2 restée sans réponse · **ADIRS aligné, PFD et ND vivants** · **séquence du §7.0 tenue si le type en impose une** · **`AUTO START` non utilisé tant que la séquence n'a pas été tenue une fois** · **mémo T.O tout vert** · **`GW − FOB` comparé à l'EZFW** · Aircraft Health regardé, relevé postérieur au 21/08 · build inchangé depuis la dernière observation du FMS · contrôle §A.5 passé si la feuille part en fichier
...
| §7.0 — rangée basse coupée sur les quatre panneaux | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 20 |
| §7.0 — emplacement de la commande `ADIRS` | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 21 |
| §7.0 — positions réelles du sélecteur `NORM` | **inventaire** : une capture studio qui les vise, et tombe avec la rangée basse si le panneau ENGINE est montré entier | REV 21 |
| §7.0 — contenu du panneau `GROUND SERVICES` | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 21 |
...
| **Périmètre de la checklist embarquée** (§7.0 bis) | ouvrir l'entrée `CHECKLIST` et lire ce qu'elle porte. Tant que ce n'est pas fait, aucune ligne du §7.0 ne se retire au motif qu'elle ferait doublon | REV 24 |
...
⇒ **Les quatre premiers sont des questions d'inventaire** : ils demandent de voir le menu entier, pas de le manipuler. Le live du studio du **28/08/2026** devait être cette occasion : il ne l'a pas été — aucun panneau Systems n'y figure (§7.0, « ce que le live a fermé »). Ils attendent une capture qui les vise, ou le menu à l'écran. Le cinquième est une question de comportement : il ne se ferme qu'aux commandes.
...
Contenu de l'entrée `CHECKLIST` ____ ⇒ décide si le §7.0 se réduit, et de combien.
...
**FIN · REV 24 · Simulation uniquement**
