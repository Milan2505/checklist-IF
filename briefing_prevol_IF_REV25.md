...
## CE QUI CHANGE DEPUIS LA REV 24

**Objet de la révision : le §7.0 cesse d'être écrit pour un seul appareil.** La REV 24 portait une section de mise en route entièrement A321neo, inutilisable sur tout autre type. Elle se scinde en un cadre valable pour toute la flotte et un bloc spécifique conditionné au type volé. Les 45 lignes de la REV 24 sont conservées **au caractère près** : elles ont changé d'adresse et gagné une condition d'entrée. Aucune cellule de cartouche ne bouge.

| # | Objet | Nature |
|---|---|---|
| 1 | **§7.0 · CADRE GÉNÉRIQUE — tous types.** Registre des types (séquence requise ou non, alignement ADIRS, pages SD vivantes, bloc spécifique), puis les dix phases écrites sans nommer un seul bouton. S'applique à n'importe quel appareil | refonte |
| 2 | **§7.0.A · BLOC SPÉCIFIQUE — A321neo.** Les 45 lignes de la REV 24, inchangées, mais désormais **conditionnées** : le bloc n'existe que si la cellule `Type` du cartouche ① vaut `A21N`. Inerte pour tout autre type | refonte |
| 3 | **La condition passe par une cellule déjà lue.** Aucun libellé nouveau, aucune clé à déclarer : `Type` est au registre depuis toujours, au cartouche ①. Le code n'a qu'une question à trancher — est-ce l'A21N, oui ou non | note |
| 4 | **§7.0 · REGISTRE DES TYPES** — cinq colonnes, destiné à grandir : plusieurs lignes sont en `[MANQUE]`. Un type absent ne se déduit pas d'un voisin, même règle qu'au §9 pour les masses à vide. Trois observations au parking créent la ligne, aucun vol nécessaire | ajout |
| 5 | §7.0 bis · MENU MISC — déclaré **propre au simulateur et non au type** : il s'applique à tous les appareils, la section le dit désormais | mise à jour |
| 6 | §8 et §9 — renvois répartis entre le cadre générique et le bloc A21N, qui ne concerne aucun autre vol. Les cinq points ouverts d'inventaire passent au §7.0.A | mise à jour |
| 7 | **Registre du parseur — une seule correction, mesurée.** Le libellé `Type` était déclaré `text` sans longueur maximale : la ligne `Type` de la table « AU TEMPS 1 » du §A.4 était **retenue**, et la cellule « donné par Milan, jamais déduit » écrasait le code du cartouche ① par la règle « la dernière l'emporte ». Les 28 autres lignes de cette table sont refusées avec motif ; celle-là passait — et c'est précisément la cellule dont dépend le §7.0.A. `max:12` posé. Aucune clé ajoutée ni retirée | correctif |
| 8 | **Page · la séquence est déroulée.** La checklist en ligne portait une ligne unique « séquence de mise en route, si le type en impose une ». Elle porte désormais une phase entière : les dix phases du §7.0 pour tous les types, et les 45 lignes du §7.0.A intercalées sous leur phase, dans l'ordre, quand `Type` vaut `A21N` | ajout |
| 9 | **§A.4 et §A.5 inchangés à l'octet.** 36 cellules, cinq cartouches, les deux portes de la REV 23 comprises. Rien de cette refonte n'entre dans un cartouche ni dans le registre du parseur | note |
...
OFP de moins de 2 h · MIN DIV et BINGO calculés · distance MCDU = distance OFP · briefing panne fait · schedule volets sorti pour le type · **36 cellules des cartouches renseignées** · **neuf champs du §0.2 bis renseignés, aucun laissé à sa valeur de démonstration** · type et masse à vide donnés par Milan, aucune alerte du §A.2 restée sans réponse · **ADIRS aligné, PFD et ND vivants** · **séquence du §7.0 tenue si le type en impose une, §7.0.A tenu si `Type` vaut `A21N`** · **`AUTO START` non utilisé tant que la séquence n'a pas été tenue une fois** · **mémo T.O tout vert** · **`GW − FOB` comparé à l'EZFW** · Aircraft Health regardé, relevé postérieur au 21/08 · build inchangé depuis la dernière observation du FMS · contrôle §A.5 passé si la feuille part en fichier
...
**FIN · REV 25 · Simulation uniquement**
