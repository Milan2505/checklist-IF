...
## CE QUI CHANGE DEPUIS LA REV 25

**Objet de la révision : trois libellés en attente depuis la REV 18 entrent au registre du parseur, et quittent donc les « repères non lus ».** `Autobrake départ`, `Autobrake arrivée` et `Altitude initiale` étaient écrits dans la feuille sans être lus par personne : la page en affichait trois cases de saisie manuelle qui restaient vides quel que soit le fichier chargé. Elles sont désormais déclarées aux deux endroits qu'il faut — registre du parseur **et** champs de la page — et elles remontent. Le §7.0 et le §7.0.A ne bougent pas.

| # | Objet | Nature |
|---|---|---|
| 1 | **`Autobrake départ` et `Autobrake arrivée` montent au cartouche ②**, qui se renomme **« Pistes, portes, autobrake et dégagement »** et passe de 8 à 10 cellules. C'est la sortie que le dossier avait annoncée en REV 23 — « il monte au cartouche ② le jour où la page sait le lire, pas avant » | ajout |
| 2 | **`Altitude initiale` monte au cartouche ③**, à côté des trois autres altitudes tenues à la main. La ligne du §A.5 devient **quatre altitudes à la main**, le cartouche passe de 14 à 15 cellules | ajout |
| 3 | **Le total passe de 36 à 39 cellules** estimées au temps 1. Tous les compteurs sont repassés : §A.3 règle 8, §A.4 (trois fois), §A.5 contrôle avant envoi, §8, et le compteur de la page. Aucun `36` de compteur ne survit | correctif |
| 4 | **Le bloc « Repères non lus » retombe à quatre lignes** : Route, Date, Immatriculation, MANQUE. La règle 11 du §A.3 redevient vraie sans exception — un libellé déclaré au registre ne peut pas figurer dans un bloc annoncé comme non lu | correctif |
| 5 | **`Immatriculation` reste en attente**, seule. Elle n'entre pas dans cette révision : le libellé `Inscription` du §0.2 bis est déjà lu et alimente la même donnée, la fusion des deux se tranche à part | note |
| 6 | ⚠ **`Altitude initiale` et `Altitude (Pieds)` sont deux cellules sans aucune parenté.** La première est l'altitude initiale de la SID, au cartouche ③ ; la seconde reste le FL de croisière, au §0.5. Deux clés distinctes au registre, deux libellés sans recouvrement : « Altitude initiale » n'atteint jamais l'entrée « Altitude », la résolution se fait par égalité stricte. Confondre les deux serait la faute la plus chère de cette révision | note |
| 7 | **Type des deux crans : texte, pas nombre.** `MAX`, `MED` et `LOW` sont autant de valeurs attendues que `1`, `2`, `3` — un type numérique aurait refusé les trois premières sans que le motif dise pourquoi. Longueur maximale de quatre caractères : un cran nu passe, `3 (MED)` est refusé avec motif | note |
| 8 | **§7.0.A · compte des sources — la correction de la REV 24 était mal motivée.** Le chiffre de 30 `[WIP]` était exact pour les 42 lignes des REV 21 à 23 ; le passage à 32 vient des deux lignes `[WIP]` ajoutées en REV 24, pas d'une erreur de comptage. Note reformulée, aucun chiffre du tableau ne bouge | correctif |
| 9 | **Le point ouvert de la règle 4 du §A.5 reste ouvert.** Cette révision ne le tranche pas : les trois nouveaux libellés s'écrivent nus, sans parenthèse et sans unité, comme tous ceux des cartouches | note |
...
8. **Ne jamais rendre une feuille avec des cases à remplir — sans exception, et « laisser vide » n'en est pas une.** Un champ que SimBrief remplit seul sort avec la valeur que SimBrief applique, écrite en clair, pour être comparée. Les 39 cellules des cartouches sortent estimées au temps 1, marquées `[CALC]` ou `[NAV]`, et se font écraser au temps 2.
   ⇒ Une donnée qui **ne peut pas être estimée** — une porte, et tout champ non estimable à venir — sort en `NIL`, valeur nue, motif en prose (§A.5 règle 7). Une case en `NIL` n'est pas une case à remplir : elle est remplie, et ce qu'elle dit est vrai.
...
**FIN · REV 26 · Simulation uniquement**
