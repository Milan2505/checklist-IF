...
## CE QUI CHANGE DEPUIS LA REV 26

**Objet de la révision : la cellule d’une porte cesse de n’avoir que deux issues.** Elle en a trois. Les bases de statut de vol servent très souvent le **terminal** alors qu’elles ne servent pas encore le poste ; jusqu’ici cette information était jetée et la cellule sortait en `NIL`, c’est-à-dire en disant « rien » alors qu’on savait quelque chose. Aucune cellule n’est ajoutée : le total reste à 39.

| # | Objet | Nature |
|---|---|---|
| 1 | **§A.4 · trois niveaux pour `Porte départ` et `Porte arrivée`**, dans cet ordre de préférence : le poste relevé (`A49`, `B27`, `50-5`) ; à défaut le terminal relevé, **préfixé `T`** (`T2`, `TA`, `TIBZ`) ; à défaut `NIL` | refonte |
| 2 | **Le terminal ne s’estime pas plus que la porte.** Il est relevé ou il ne l’est pas. Ni le terminal habituel de la compagnie, ni celui d’un autre vol du même exploitant — même au même terrain, le même jour | note |
| 3 | **Aucune clé `Terminal départ` ni `Terminal arrivée`.** Le cartouche ② passerait de dix à douze cellules pour une information qui n’existe que par défaut d’une autre : deux cellules vides la plupart du temps, exactement le défaut qu’on ferme. Le terminal vit dans la cellule de la porte | note |
| 4 | **Registre du parseur — `max:6` sur les deux clés.** `TIBZ` fait quatre caractères, un poste comme `50-5` aussi, `T2-15` cinq : six couvrent les deux formes et refusent toute phrase. La seule borne était jusqu’ici le motif `code` à douze caractères. Aucune clé ajoutée ni retirée | correctif |
| 5 | **Rendu — un terminal est une valeur relevée**, pas une absence : il s’affiche en valeur pleine comme un poste, jamais dans l’habillage grisé de `NIL`. Une ligne de légende l’annonce au bas de la page | mise à jour |
| 6 | **§A.5 règle 7 inchangée.** `NIL` garde son sens et reste le dernier recours ; c’est le chemin qui y mène qui gagne une étape, pas la règle | note |
...
**FIN · REV 27 · Simulation uniquement**
