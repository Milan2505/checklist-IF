...
## CE QUI CHANGE DEPUIS LA REV 27

**Objet de la révision : la page cesse d'afficher autre chose que ce que la feuille porte.** Trois cellules de cartouche étaient réécrites à la lecture — les deux caps et les deux masses de sortie. Deux le sont encore, mais volontairement et à l'identique. Aucune cellule n'est ajoutée : le total reste à 39.

| # | Objet | Nature |
|---|---|---|
| 1 | **`Cap départ` et `Cap dégagement` — genre `cap`.** Ils étaient lus comme des entiers, donc passés par une conversion numérique qui mangeait le zéro initial : une feuille portant `043` faisait afficher `43`. Un cap s'écrit sur trois chiffres et ne sert à aucun calcul dans la page. Il est désormais validé comme un code, refusé au-delà de 360, et rendu sur trois chiffres — `7` devient `007` | correctif |
| 2 | **Le défaut ne s'est révélé que par hasard**, sur U21703, première feuille du dossier à porter un cap sous 100. Aucune des vingt autres cellules numériques n'est concernée : une altitude, une masse, une vitesse, un carburant ou une distance ne s'écrit pas avec un zéro de tête | note |
| 3 | **`TOW` et `LW` s'affichent en kilos.** Ils étaient convertis en tonnes à la lecture — `62260` devenait `62.3`. C'étaient les deux seules cellules dont le nombre affiché n'était pas celui de la feuille, alors que le contrôle avant envoi du §A.5 demande de comparer l'un à l'autre. La page ne calcule rien avec ces masses : elle les affiche, en kilos | correctif |
| 4 | **La table Vref du §6.2 reste en tonnes.** C'est une lecture humaine, pas une conversion que la page doit faire à la place du pilote. La ligne de checklist le dit désormais en clair | note |
| 5 | **§A.5 règle 1 — la tolérance du parseur est écrite.** Sur une cellule numérique, il retire l'espace de milliers et l'unité finale avant de valider : `6 250` et `250 kg` passent. La règle n'est pas assouplie ; elle cesse de promettre un refus qui n'arrive pas. Une feuille qui s'appuie sur cette tolérance est hors contrat, et rien ne le signalera | mise à jour |
| 6 | **Revue des 39 clés — ce qui reste réécrit à la lecture.** Les onze cellules `code` et les quatre `text` ressortent telles quelles. Les deux `cap` se rendent sur trois chiffres, par construction. Les vingt-deux cellules numériques perdent l'espace de milliers et l'unité finale quand la feuille en met, ce qui est la tolérance de la règle 1. Plus aucune conversion d'unité nulle part | note |
| 7 | **Aucun compteur ne bouge.** 39 cellules, cinq cartouches, cartouche ② à dix, cartouche ③ à quinze | note |
...
**FIN · REV 28 · Simulation uniquement**
