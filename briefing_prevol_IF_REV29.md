...
## CE QUI CHANGE DEPUIS LA REV 28

**Objet de la révision : ce que le dossier savait du code cesse de vivre dans un changelog.** La REV 28 avait établi le genre des 39 clés — onze `code`, quatre `text`, deux `cap`, vingt-deux numériques — mais l'a écrit dans sa table « ce qui change », qui est remplacée à chaque révision. Cet inventaire serait mort ici. Il descend au §A.5, où il devient une table permanente. Aucune cellule n'est ajoutée : le total reste à 39.

| # | Objet | Nature |
|---|---|---|
| 1 | **§A.5 · GENRE DES 39 CLÉS** — table permanente, une ligne par groupe de genre, comptes en tête. Elle dit ce que le parseur fait de chaque cellule : ce qui ressort tel quel, ce qui se rend sur trois chiffres, ce qui perd l'espace de milliers et l'unité finale. C'est la seule page du dossier qui décrive le code plutôt que la procédure | ajout |
| 2 | **La répartition clé par clé est reconstruite, pas lue.** Les comptes viennent de la revue REV 28 et font foi ; l'affectation de chaque libellé à son genre est une reconstitution à partir de ce que chaque cellule porte. Elle se confirme contre `js/data/fields.js`, et toute ligne fausse se corrige en place — si un compte ne tombe plus, c'est la répartition qui est fausse, jamais le compte | note |
| 3 | **§A.4 · la feuille reprend la numérotation du §0 du dossier.** Elle n'était écrite nulle part, et deux feuilles du 31/08/2026 sont sorties avec deux numérotations différentes : `EW9743` ouvre sur un §0.1 « identification et équipement — priorité » supprimé depuis la REV 22, `U21703` suit l'ordre gelé. Une feuille qui renumérote n'est pas mal lue par la page — elle est illisible pour l'œil qui descend le formulaire en parallèle | correctif |
| 4 | **Le gabarit est déclaré dérivé.** Il n'est pas une source : il se refait à partir du dossier à chaque révision qui touche le §0, le §A.4 ou le §A.5, et il ne porte aucune règle que le dossier ne porte pas déjà. Le gabarit REV 27 portait les deux défauts du point 3 et une note « quel dossier fait foi » tranchée depuis la REV 26 | correctif |
| 5 | **§8 · un point ouvert de plus, et il est bon marché.** Le genre des 39 clés se confirme par une lecture de `js/data/fields.js`, sans voler et sans capture. Il rejoint le point de la parenthèse de traduction, ouvert depuis la REV 19, que la même lecture peut fermer | ajout |
| 6 | **Aucun compteur ne bouge.** 39 cellules, cinq cartouches, cartouche ② à dix, cartouche ③ à quinze | note |
...
1. **Valeur nue.** `6250`, pas `6 250 kg`, pas `6250 kg (bloc)`. Ni unité, ni espace de milliers, ni commentaire.
   ⚠ **Ce que le parseur tolère, et qui n'est pas une licence.** Sur une cellule numérique, il retire l'espace de milliers et l'unité finale avant de valider : `6 250` et `250 kg` sont acceptés et ressortent `6250` et `250`. La règle n'en est pas assouplie — elle est écrite ici pour que le dossier dise ce que le code fait, plutôt que de promettre un refus qui n'arrive pas. Une feuille qui s'appuie sur cette tolérance est hors contrat, et rien ne le signalera.
   ⇒ Ce qui n'est **pas** toléré et se refuse avec motif : un second nombre dans la cellule (règle 2), une décimale sur un champ entier, un commentaire entre parenthèses, un marqueur hors des six retirés à la lecture.
...
|---|---|---|---|
| `cap` | 2 | rendu sur trois chiffres, refusé au-delà de 360 — `7` devient `007`, `043` reste `043` | Cap départ · Cap dégagement |
...
⚠ **Aucune cellule ne sort convertie.** Ni tonnes, ni arrondi, ni reformatage : `TOW` et `LW` s'affichent en kilos, comme la feuille les écrit. La table Vref du §6.2 reste en tonnes parce qu'elle est lue par un humain, pas par la page.
...
**FIN · REV 29 · Simulation uniquement**
