...
## CE QUI CHANGE DEPUIS LA REV 29

**Objet de la révision : le dossier cesse de promettre des refus qui n'arrivent pas.** Un audit du code, mené sur le dépôt et rejoué à l'exécution, a montré que quatre affirmations du §A.5 décrivaient un parseur plus sévère que le vrai. Deux d'entre elles couvrent des chemins par lesquels **une valeur fausse est lue en silence** — exactement la classe de défaut que le §A.5 dit être pire qu'une case vide. Aucune cellule n'est ajoutée : le total reste à 39.

| # | Objet | Nature |
|---|---|---|
| 1 | **§A.5 règles 1 et 2 — deux nombres séparés par un blanc sont soudés, pas refusés.** `digits()` retire **tous** les blancs avant le seul test d'atomicité : `1 100 1 050` ressort `11001050`, accepté, sans motif et sans bandeau. L'exemple du dossier était bien refusé, mais pour une autre raison — des lettres et un tiret y survivaient. Aggravant : `cleanCell` retire marqueurs et astérisques d'abord, donc il peut créer l'adjacence. Seule `Altitude initiale` l'intercepte, par son `max:5`, sous un motif qui ne nomme pas la cause | correctif |
| 2 | **§A.5 · la borne 360 du cap ne tient pas sur le chemin prose.** `validate()` refuse bien `400`, puis `scanProse` réécrit la clé précisément **parce qu'elle vient d'être refusée** — sa condition est « la clé est vide ». Le motif de prose capture trois chiffres sans borne et sans rendu sur trois chiffres, et la case étant remplie, l'alerte ne la marque plus non résolue. Le refus devient invisible | correctif |
| 3 | **§A.5 · « aucune cellule ne sort convertie » était faux tel qu'écrit.** La phrase, ajoutée en REV 29, vaut pour les 39 cellules de cartouche. Elle ne vaut pas pour la clé `Altitude (Pieds)` du §0, rendue en niveau de vol : `35000` s'affiche `350`, relevé sur les deux feuilles du dépôt. La phrase est restreinte et la conversion nommée | correctif |
| 4 | **§A.5 · le bandeau d'anomalie est conditionnel.** Seuls les champs requis y entrent, seul le premier refus par clé est nommé, un refus est jeté si la clé a été résolue ailleurs, et sans aucun cartouche le bandeau est masqué. Le dossier écrivait « affiché au bandeau » comme une garantie | mise à jour |
| 5 | **§A.5 · les marqueurs, deux angles morts.** Les six retirés à la lecture ne le sont qu'en majuscules et crochets collés — `[ofp]` n'est pas retiré. Un marqueur sans crochet fermant ou entre parenthèses traverse un champ `text`. Et un marqueur collé à un **libellé** empêche la résolution, sans que rien ne le dise | mise à jour |
| 6 | **§A.5 · un dossier ne remplit aucune case.** L'extraction n'est jamais appelée sur `briefing.md` : seule une feuille alimente la checklist. Le récit du correctif `max:12` de la REV 25 désignait donc la mauvaise cellule — le correctif est juste, son explication ne l'était pas | note |
| 7 | **§8 · quatre points ouverts de plus**, dont trois sont des correctifs de code déjà écrits et non appliqués, et un est une décision à prendre sur la conversion en niveau de vol | ajout |
| 8 | **Aucun compteur ne bouge.** 39 cellules, cinq cartouches, cartouche ② à dix, cartouche ③ à quinze | note |
...
**Présent, parce que rien ne le produit :** l'état réel du FMS d'IF, les seuils de décision carburant, le briefing panne, les minima chartés, le schedule volets, les critères de stabilisation, les distances d'annonce, le déroutement à la main, les procédures propres au simulateur, la séquence de mise en route quand le type en impose une, et le contrat de lecture de la feuille.

⚠ Le §0 est l'exception assumée : il précède SimBrief, donc il ne peut rien répéter.

⚠ **Une entrée `CHECKLIST` figure au menu MISC des captures studio (§7.0 bis).** Si elle porte une checklist cold & dark, la troisième liste ci-dessus perd sa dernière ligne et le §7.0 comme le §7.0.A se réduisent à ce que la checklist embarquée ne dit pas. La règle ne s'applique pas sur la foi d'un bouton : elle attend que son contenu soit lu.
...
⚠ **Six marqueurs sur huit sont retirés à la lecture** — `[OFP]` `[FORM]` `[NAV]` `[SIM]` `[RDR]` `[CALC]`. Les trois autres ne le sont pas, et chacun a sa réponse : `[WIP]` disqualifie la cellule entière, `[MANQUE]` se lit « non fourni », `[RÉEL]` — comme tout marqueur qu'une révision ajouterait — **fait rejeter la cellule avec un motif qui le nomme**. La règle « ces trois-là ne s'écrivent que dans la PARTIE B » n'est donc plus seulement rédactionnelle : elle est tenue par le parseur (§A.5).
⚠ **Le motif existe, son affichage n'est pas garanti.** Le bandeau ne montre le refus que si la clé est requise, que c'est le premier refus sur cette clé, qu'elle n'a pas été résolue ailleurs, et qu'au moins un cartouche a été lu. Un refus peut donc être juste et muet : ne jamais écrire une feuille en comptant sur le bandeau pour rattraper une erreur de format.
...
8. **Ne jamais rendre une feuille avec des cases à remplir — sans exception, et « laisser vide » n'en est pas une.** Un champ que SimBrief remplit seul sort avec la valeur que SimBrief applique, écrite en clair, pour être comparée. Les 39 cellules des cartouches sortent estimées au temps 1, marquées `[CALC]` ou `[NAV]`, et se font écraser au temps 2.
   ⇒ Une donnée qui **ne peut pas être estimée** — et, pour une porte, seulement après que le terminal a été cherché lui aussi (§A.4) — sort en `NIL`, valeur nue, motif en prose (§A.5 règle 7). Une case en `NIL` n'est pas une case à remplir : elle est remplie, et ce qu'elle dit est vrai.
...
10. Un libellé ne s'ajoute à un cartouche qu'une fois inscrit **au registre du parseur ET à la liste des champs de la page**. Dans cet ordre, jamais l'inverse : un libellé écrit mais non déclaré ne fait pas remonter sa valeur, et rien ne le signale (§A.5).
   ⚠ **Une inscription à moitié ne vaut rien.** Une clé déclarée au registre mais absente de la liste des champs est bien lue dans la feuille, puis jetée : aucune case ne la reçoit, aucun bandeau ne le dit, et seul un contrôle de démarrage la signale en console. C'est le même défaut silencieux que la règle prétend empêcher, une marche plus bas. La règle ne disait « registre » qu'au singulier jusqu'à la REV 23.
...
12. **Ce qui est montré par le studio n'est pas ce qui vole.** Une capture WIP, un post ou un changelog se marque `[WIP]` et reste dans une section dormante jusqu'à la vérification à l'écran. Un correctif de changelog ne retire pas une ligne du dossier : seule une observation le fait.
    ⇒ Corollaire posé en REV 24 : **du matériel studio abondant n'est pas du matériel studio dirigé.** Deux captures d'un live peuvent ouvrir cinq sujets neufs sans fermer un seul point ouvert du dossier — c'est exactement ce qui s'est produit le 29/08/2026. Ce qu'on voit ne se choisit pas ; ce qu'on en tire, si.
...
**TEMPS 1** : la feuille SimBrief, dans l'ordre du formulaire, en trois colonnes — champ, ce qui se tape, valeur attendue. Rien d'autre : pas de météo, pas de commentaire de vol. Elle se termine par les cinq cartouches ci-dessous, **39 cellules remplies d’estimations**, chacune avec sa source rangée en prose sous le tableau.

⚠ **La feuille reprend la numérotation du §0 du dossier, section par section.** `§0.1 Informations sur le vol`, `§0.2 Informations sur l'avion — partie visible`, `§0.2 bis` pour les neuf champs repliés, puis `§0.3` à `§0.10`. Aucune feuille ne renumérote, n'ajoute une section, ni ne remonte un bloc en tête sous prétexte de priorité : le titre « identification et équipement — priorité » est supprimé depuis la REV 22 et ne revient pas.
⇒ Motif : la feuille se tape en descendant le formulaire à l'écran. Une numérotation qui ne suit pas celle du dossier n'est pas rejetée par la page — elle fait sauter des cases à l'œil, ce qui est exactement le défaut que l'ordre gelé du §0 ferme.
⚠ **Le gabarit vierge est un dérivé du dossier, jamais une source.** Il se refait à chaque révision qui touche le §0, le §A.4 ou le §A.5, il porte le numéro de cette révision, et il n'énonce aucune règle qui ne soit pas déjà ici. Un gabarit qui contredit le dossier est périmé, pas concurrent.
...
**② Pistes, portes, autobrake et dégagement — 10 cellules**
...
**③ Vitesses et trajectoire — 15 cellules**
...
⚠ Ce bloc ne contient **aucun libellé du registre** (§A.3 règle 11). La REV 26 en a sorti trois lignes : le parseur sait désormais lire `Autobrake départ`, `Autobrake arrivée` et `Altitude initiale`, elles sont donc montées aux cartouches ② et ③. C’était la sortie annoncée depuis la REV 23 — « il monte au cartouche ② le jour où la page sait le lire, pas avant ». Les quatre qui restent ne sont toujours lues par rien.

⇒ Origine de chaque ligne **au temps 2** : V1, VR, V2, Flex et Volets du Runway Analysis `[OFP]` ; Réduction, Accélération et Transition posées à la main (§4) ; ILS, DA, Inbound et Point inbound des cartes `[NAV]` et de la STAR retenue (§6.6) ; Vref et Vapp calculées sur le LW `[CALC]` (§6.2) ; BLOC, EXTRA et FUEL FACTOR de l'OFP `[OFP]` ; BINGO et MIN DIV calculés (§3) ; les six masses de la page de garde `[OFP]`, sauf la masse à vide IF qui vient de Milan `[SIM]` ; les six lignes de pistes et de dégagement du cartouche ② de l'OFP et des cartes ; **les deux portes du cartouche ② restent `[RDR]` — poste, à défaut terminal préfixé `T`, à défaut `NIL`** — l’OFP n’en porte aucune des trois formes, et aucune ne devient `[OFP]` au temps 2 ; les deux crans d’autobrake et l’altitude initiale décidés au briefing (§4), désormais au cartouche.

**AU TEMPS 1 — comment j’estime les 39**
...
| Porte départ | poste relevé sur le traceur de trafic `[RDR]` ; à défaut le terminal relevé, préfixé T ; à défaut `NIL` |
| Piste arrivée | piste attendue au vent prévu à l'ETA `[NAV]` |
| Porte arrivée | poste relevé sur le traceur de trafic `[RDR]` ; à défaut le terminal relevé, préfixé T ; à défaut `NIL` |
...
| Altitude initiale | première altitude de la SID, à défaut élévation terrain + 3 000 ft `[NAV]` |
...
⚠ **Une porte ne s’estime pas — et un terminal pas davantage.** Aucune méthode de calcul n’existe pour l’un ni pour l’autre : ce sont des données observées, ou rien. Pas de déduction depuis la compagnie, ni depuis l’habitude d’une ligne, ni depuis un autre vol du même exploitant — même au même terrain, le même jour.
⚠ **TROIS NIVEAUX, dans cet ordre de préférence, dans la MÊME cellule.** Les bases de statut servent très souvent le terminal alors qu’elles ne servent pas encore le poste, et un terminal connu vaut mieux qu’une cellule qui dit « rien ».

| Ce qui est relevé | Ce que porte la cellule | Exemples |
|---|---|---|
| le poste | la porte, telle quelle | `A49` · `B27` · `50-5` |
| le terminal seul, poste non publié | le terminal **préfixé `T`** | `T2` · `TA` · `TIBZ` |
| ni l’un ni l’autre | `NIL` | |

⇒ **Aucune clé `Terminal départ` ni `Terminal arrivée` n’est créée.** Le cartouche ② resterait à dix cellules douze fois sur dix : le terminal n’existe que par défaut de la porte, il vit donc dans la cellule de la porte. Deux cellules vides la plupart du temps, c’est le défaut qu’on ferme, pas qu’on déplace.
⇒ Un terminal est une donnée **relevée** : la page l’affiche en valeur pleine, comme un poste, et non dans l’habillage grisé réservé à `NIL`.
...
⚠ Aucune de ces 39 n’est une valeur de dispatch. Elles existent pour que la checklist s'affiche complète et pour donner un ordre de grandeur à comparer.
...
1. **Valeur nue.** `6250`, pas `6 250 kg`, pas `6250 kg (bloc)`. Ni unité, ni espace de milliers, ni commentaire.
   ⚠ **Ce que le parseur tolère, et qui n'est pas une licence.** Sur une cellule numérique, il retire **tous les blancs** puis l'unité finale avant de valider : `6 250` et `250 kg` sont acceptés et ressortent `6250` et `250`. La règle n'en est pas assouplie — elle est écrite ici pour que le dossier dise ce que le code fait, plutôt que de promettre un refus qui n'arrive pas. Une feuille qui s'appuie sur cette tolérance est hors contrat, et rien ne le signalera.
   ⚠ **Ce n'est pas « l'espace de milliers » qui est retiré, c'est tout blanc — et c'est fait avant le test d'atomicité.** Deux nombres séparés par un blanc seul ne sont donc pas refusés : ils sont **soudés et acceptés**. `1 100 1 050` ressort `11001050`, sans motif et sans bandeau. `cleanCell` retirant marqueurs et astérisques en premier, il peut lui-même créer l'adjacence : `64 460 [CALC] 77 800` devient `64 460 77 800`, donc `6446077800`. Seule `Altitude initiale` s'en protège, par son `max:5`, et sous un motif qui ne nomme pas la cause.
   ⇒ Ce qui se refuse réellement avec motif : une cellule où survit autre chose qu'un chiffre — lettre, tiret, ponctuation, commentaire entre parenthèses — une décimale sur un champ entier, un marqueur hors des six retirés à la lecture, un dépassement de longueur maximale.
   ⇒ Point ouvert du §8 jusqu'au correctif : tester l'atomicité **avant** d'écraser les blancs, ou ne retirer un blanc qu'entre deux chiffres d'un même nombre.

2. **Une cellule, une valeur.** `64460`, et non `64 460 kg — MLW 77 800, marge 13 340`. Les marges vont en prose, sous le tableau.
   ⚠ **Cette règle est tenue par celui qui écrit, pas par le parseur.** Une cellule à deux nombres n'est rejetée que si quelque chose d'autre qu'un chiffre y survit — c'est le cas de l'exemple ci-dessus, à cause des lettres et du tiret. Deux nombres nus séparés par un blanc passent, soudés (règle 1). Le refus promis par les révisions 19 à 29 n'existe pas pour ce cas-là.
...
| Autobrake départ · Autobrake arrivée | crans retenus | cartouche ② |
| Dégagement · Cap dégagement · Distance dégagement | déroutement | cartouche ② |
| V1 · VR · V2 · Flex · Volets | décollage | cartouche ③ |
| Altitude initiale · Réduction · Accélération · Transition | quatre altitudes à la main | cartouche ③ |
...
**Genre des 39 clés** — ce que le parseur fait de la valeur, une fois le libellé résolu. Cette table décrit le code, pas la procédure : elle est ici pour que le dossier cesse de promettre ce que le code ne fait pas.

| Genre | n | Ce qui sort | Clés |
|---|---|---|---|
| `cap` | 2 | rendu sur trois chiffres, refusé au-delà de 360 — `7` devient `007`, `043` reste `043`. Borne non tenue sur le chemin prose, voir ci-dessous | Cap départ · Cap dégagement |
| `code` | 11 | tel quel, aucune réécriture | Callsign · Compagnie · Type · Piste départ · Piste arrivée · Porte départ · Porte arrivée · Dégagement · Volets · ILS · Point inbound |
| `text` | 4 | tel quel, aucune réécriture | Autobrake départ · Autobrake arrivée · Flex · FUEL FACTOR |
| numérique | 22 | l'espace de milliers et l'unité finale sont retirés avant validation (règle 1), aucune conversion d'unité | V1 · VR · V2 · Altitude initiale · Réduction · Accélération · Transition · DA · Vref · Vapp · Inbound · Distance dégagement · BLOC · BINGO · MIN DIV · EXTRA · Masse à vide IF · Charge utile · Poids zéro carburant · Trip · TOW · LW |

⚠ **Les comptes font foi, la répartition est reconstruite.** `11 / 4 / 2 / 22` vient de la revue des clés menée en REV 28, sur le code. L'affectation de chaque libellé à son genre, elle, a été reconstituée à partir de ce que la cellule porte — elle n'a pas été lue dans `js/data/fields.js`. Trois lignes sont des choix de reconstitution assumés et sont les premières à vérifier : `Flex`, qui peut porter une température comme une mention, `ILS`, qui porte une fréquence à décimale, et `Point inbound`, qui porte un repère nommé ou un DME.
⇒ Une ligne fausse se corrige ici, en place. Si un compte ne tombe plus après correction, c'est la répartition qui est fausse — jamais le compte.
⚠ **Aucune des 39 cellules de cartouche ne sort convertie.** Ni tonnes, ni arrondi, ni reformatage : `TOW` et `LW` s'affichent en kilos, comme la feuille les écrit. La table Vref du §6.2 reste en tonnes parce qu'elle est lue par un humain, pas par la page.
⚠ **Une clé convertit, et elle n'est pas dans les 39 : `Altitude (Pieds)`, au §0.** Elle est rendue en niveau de vol — `35000` s'affiche `350`, `33000` s'affiche `330`, relevé sur les deux feuilles du dépôt. La feuille continue de porter le niveau en pieds, chiffré, comme le §0.4 le demande : c'est l'affichage qui traduit, pas la saisie. Tant que ce comportement n'est pas tranché comme voulu ou non, la phrase ci-dessus ne s'étend pas à lui.
⚠ **La borne 360 du cap ne protège que la cellule, pas la case.** `validate()` refuse bien un cap au-delà de 360 ; puis le balayage de la prose réécrit la clé, **parce qu'elle vient d'être refusée** — sa condition est « la clé est vide », et un refus laisse la clé vide. Le motif de prose capture trois chiffres sans borne et sans rendu sur trois chiffres, et la case une fois remplie n'est plus signalée comme non résolue. Conséquence pour qui écrit : **un cap faux dans la cellule et repris en prose s'affiche depuis la prose, sans un mot.** La prose ne doit jamais porter un cap qui contredit la cellule.
⇒ Point ouvert du §8 jusqu'au correctif : le balayage de prose doit distinguer une clé **absente** d'une clé **refusée**, et ne jamais réécrire la seconde.
⇒ `NIL` et un terminal préfixé `T` traversent tous les genres : ce sont des valeurs nues, pas des cas particuliers du parseur.

**Marqueurs — ce que le retrait attrape vraiment**

⚠ Les six marqueurs retirés à la lecture ne le sont **qu'en majuscules et crochets collés**. `[ofp]` n'est pas reconnu : la cellule part en refus, nommé — donc pas silencieux, mais refusé quand même. Les marqueurs s'écrivent en majuscules, sans espace dans les crochets.
⚠ Le filet des marqueurs inconnus **exige le crochet fermant**. `[RÉEL` sans fermeture, ou `(RÉEL)` entre parenthèses, traverse un champ `text` et s'affiche tel quel dans la case.
⚠ **Un marqueur collé à un libellé empêche la résolution, sans rien afficher.** `Cap départ [NAV]` en colonne 1 n'est pas nettoyé : la clé n'est pas reconnue, la valeur n'atteint aucune case, et rien ne le dit. Les marqueurs vivent en colonne 2 ou en prose, jamais sur le libellé.
⇒ Point ouvert du §8 jusqu'au correctif : insensibilité à la casse et aux espaces sur les six, fermeture non exigée sur le filet des inconnus, nettoyage du libellé avant résolution.

**Un dossier ne remplit aucune case**

⚠ L'extraction n'est **jamais** appelée sur `briefing.md` : seule une feuille de vol alimente la checklist. Le dossier peut donc contenir un libellé du registre suivi d'un nombre sans qu'aucune case bouge — la règle 5 reste une règle de feuille, et les tables de méthode du §A.4 ne sont pas un danger de lecture.
⇒ C'est ce qui corrige le récit du correctif `max:12` posé en REV 25. Le correctif est juste et fait son travail : sans lui, trois lignes du dossier résolvent sur la clé `Type`. Mais le gagnant de « la dernière l'emporte » serait le libellé du §0 `Type d'aéronef (Aircraft)`, pas la ligne de la table « AU TEMPS 1 » — et sur un dossier, aucun des deux ne remplit quoi que ce soit.
...
⇒ **La page racine `MCDU MENU` porte quatre entrées** : `<FMGC`, `<ATSU`, `<AIDS`, `<CFDS`, avec un `HELP HINTS` commutable. Le FMGC n'est donc pas la page d'ouverture : il se sélectionne. `[WIP]`, relevé sur capture studio du 29/08/2026 — sur un poste A320 famille dont le type exact n'est pas identifié.
...
⚠ Un vol conduit avec l'accélération temporelle (§7.0 bis) ne produit aucun relevé exploitable : les points du navlog défilent sans que la consommation soit vécue. Le §9 se remplit sur un vol en temps réel, ou pas du tout.
...
⚠ **RIEN ICI NE S'APPLIQUE AUJOURD'HUI.** Le mode n'a été vu que sur une capture studio du 29/08/2026, sur un poste A320 famille dont le type n'est pas identifié. Il n'existe pas dans le build de Milan. La section est écrite pour être réveillée, pas pour être suivie — sur le modèle du §7.0, dormant en REV 20 puis rempli en REV 21.

⚠ Numérotée **6.7** et non 6.4 : la stabilisation occupe déjà le 6.4. Le §6 va désormais jusqu'à 6.7.

**CE QUI A ÉTÉ LU SUR LA CAPTURE** `[WIP]`

Bandeau FMA : `SPEED` · `G/S` · `LOC` · `CAT3 DUAL` · `AP1+2` · `1FD2` · `A/THR`.
Deux boutons `AP ON` distincts sur la planche autopilote, là où le build actuel n'en porte qu'un.
Paramètres au moment de l'image : 1 024 ft AGL · 144 kt · volets FULL · train sorti · aérofreins armés · N1 52 % · THS 20 %.

**CE QUE LE FMA DOIT DIRE, LE JOUR OÙ IL SERA LU EN SESSION**

| Case | Attendu | Ce que ça veut dire |
|---|---|---|
| `LOC` puis `G/S` | en vert | axe et plan capturés, dans cet ordre |
| `CAT3 DUAL` | affiché | double chaîne annoncée **par l'avion** — pas par la piste, pas par la météo |
| `AP1+2` | affiché | les deux pilotes automatiques engagés. Un seul AP ne donne pas la CAT III |
| `A/THR` | actif | sans lui, pas d'atterrissage automatique |

⚠ **Une capacité annoncée n'est pas une autorisation.** Le simulateur ne modélise ni les minima chartés de la piste, ni l'état des équipements sol. `CAT3 DUAL` affiché dans une visibilité de 200 m reste une manœuvre non couverte par une carte : le §6.4 et les minima du §6 continuent de s'appliquer tels quels.

**EFFET SUR LE CARTOUCHE ③ — TRANCHÉ D'AVANCE**

La cellule de décision d'atterrissage du cartouche ③ reste obligatoire et garde sa méthode du §A.4. Une approche automatique ne change pas cette cellule.
⇒ Motif : la CAT III réelle n'a pas de hauteur de décision mais une hauteur d'alerte, qu'aucune valeur observable du simulateur ne produit. Basculer la cellule sur une grandeur inexistante ferait lire un chiffre inventé à la checklist (§A.5).
⚠ Aucune cellule de cartouche ne change tant que cette section dort. C'est la seule chose que cette révision garantit sur ce sujet.

**CE QUI RÉVEILLE LA SECTION**

| Point | Comment il se ferme |
|---|---|
| Le mode existe dans le build de Milan | un vol : le FMA affiche `CAT3 DUAL` ou il ne l'affiche pas |
| Deux AP engageables séparément | deux boutons sur la planche, ou un seul |
| Comportement à l'arrondi automatique | un atterrissage, relevé au LANDING REPORT (§9) |
| Comportement en cas de perte d'un AP en finale | à provoquer une fois, hors trafic |

⇒ Tant que la première ligne n'est pas fermée, les trois autres n'ont pas de sens.
---

## 7 · PROPRE AU SIMULATEUR

### 7.0 · MISE EN ROUTE — CADRE GÉNÉRIQUE, TOUS TYPES

Cette section vaut pour n'importe quel appareil du simulateur. Elle ne nomme aucun bouton : elle dit ce qu'il faut obtenir, pas où appuyer. Les commandes propres à un type vivent dans un bloc spécifique, et il n'en existe qu'un aujourd'hui (§7.0.A).

**TROIS QUESTIONS, DANS CET ORDRE**

1. Le type volé impose-t-il une séquence de mise en route ? → registre ci-dessous.
2. Si oui, les dix phases s'appliquent, telles quelles.
3. Un bloc spécifique existe-t-il pour ce type ? → colonne de droite du registre. Sinon, les dix phases suffisent et les commandes se cherchent à l'écran.

**REGISTRE DES TYPES**

| Type | Séquence requise | Alignement ADIRS bloquant | Pages SD vivantes | Bloc spécifique |
|---|---|---|---|---|
| A318 · A319 · A320 (release) | non | non | non | — |
| A321 (classique) | non | non | non | — |
| A330 · A330-900neo | oui `[SIM]` | oui `[SIM]` | oui `[SIM]` | aucun — dix phases seules |
| A321neo (`A21N`) | oui `[WIP]` | oui `[WIP]` | oui `[WIP]` | **§7.0.A** |
| Boeing, Embraer, autres | `[MANQUE]` | `[MANQUE]` | `[MANQUE]` | — |

⚠ **Type absent de ce registre : on ne suppose rien.** Le vol se conduit sans séquence, et ce qui a été observé au parking se note au §9 pour créer la ligne. Un type ne rejoint pas ce tableau par ressemblance avec un autre — même règle qu'au §9 pour les masses à vide, et pour le même motif.
⇒ Les trois autres registres par type du dossier vivent au §6 : VFE (§6.1), Vref (§6.2), limite de tail strike (§6.3). Le tableau des trous en tête du §6 dit lesquels manquent.

**LES DIX PHASES** — ordre valable sur tout Airbus, et vraisemblablement au-delà.

| Phase | Objet | Attendu observable avant de passer à la suivante |
|---|---|---|
| 1 | Alimentation initiale | barres alimentées, avionique vivante, sans décharger les batteries |
| 2 | Préparation cockpit | alignement inertiel lancé, PFD et ND vivants, FMS accessible |
| 3 | Mise en route APU | APU disponible, page APU appelée au SD si le type en a une |
| 4 | Libération de l'alimentation sol | barres tenues sans le groupe de parc, mémo d'alimentation externe tombé |
| 5 | Carburant | pression établie des deux côtés, chaque aile alimentant son moteur |
| 6 | Conditionnement avant démarrage | air de démarrage disponible, conditionnement coupé |
| 7 | Démarrage moteurs | les deux moteurs stabilisés au ralenti, aucun paramètre hors plage |
| 8 | Après démarrage | générateurs moteurs en ligne, prélèvements et conditionnement rétablis, APU coupé |
| 9 | Configuration décollage | mémo T.O tout vert, T.O CONFIG passé, mémos sol tombés |
| 10 | Alignement et lâcher des freins | transpondeur, feux, A/THR armé — dans cet ordre, à l'alignement |

⚠ **La phase 2 conditionne tout le reste.** Sur un type dont la ligne « alignement ADIRS bloquant » dit oui, rien de ce qui se lit sur PFD ou ND n'est valable avant la fin de l'alignement. Sur un type dont elle dit non, la phase 2 se réduit à la préparation FMS.
⚠ **Une phase n'est pas une case à cocher, c'est un état à obtenir.** Le simulateur ne dira pas qu'elle est manquée : il affichera simplement un écran vide, un mémo qui reste, ou un item du mémo T.O qui refuse le vert.
⇒ **Le temps se budgète.** Un type à séquence ajoute un poste avant l'EOBT, comme le roulage (§A.2).

**CE QUI NE DÉPEND PAS DU TYPE**

Le menu MISC (§7.0 bis), le mémo T.O, l'Aircraft Health, le comportement d'appel automatique des pages SD et la conduite du roulage sont propres au simulateur, pas à l'appareil. Ils ne se dupliquent pas par type.

---

### 7.0.A · BLOC SPÉCIFIQUE — A321neo (`A21N`)

⚠ **CONDITION D'ENTRÉE : la cellule `Type` du cartouche ① vaut `A21N`.** Pour tout autre type, ce bloc est inerte — ni lu, ni affiché, ni tenu. C'est la seule question que le code a à trancher sur cette section : est-ce l'A21N, oui ou non. Aucun libellé nouveau n'est créé pour ça, `Type` est au registre du parseur depuis l'origine. Si `Type` ne résout pas, le bloc ne s'affiche pas — un type inconnu n'active jamais un bloc spécifique.
...
**LES DIX PHASES** — ce sont celles du §7.0, sans changement. Le §7.0.A n'en redéfinit aucune : l'entier de la colonne « Ordre » y renvoie, et `3.2` est la deuxième ligne de la phase 3.
...
| 1.3 | ELECTRICAL | `EXT PWR` | ON si le groupe de parc est branché, sinon laisser OFF | alimentation externe en ligne, mémo `EXT PWR` affiché à l'E/WD — ou décision prise de partir sur l'APU, et on saute en 3.1 | `[WIP]` `[RÉEL]` |
...
| 2.1 | hors des quatre panneaux relevés | `ADIRS` `[MANQUE]` | NAV | **alignement lancé, et lancé tôt.** Mémo `IR IN ALIGN 2 MN` annoncé à l'E/WD (§7 · ADIRS). Sur A320 réel il demande une dizaine de minutes : il tourne pendant le briefing et le chargement. PFD et ND vivants avant le repoussage | `[MANQUE]` `[RÉEL]` |
| 2.2 | hors des quatre panneaux relevés | PFD et ND | contrôle | écrans vivants, plus de drapeau ATT/ALT/SPD/VS/HDG, mémo `IR IN ALIGN` disparu, `HDG MAP NOT AVAIL` et `GPS PRIMARY LOST` effacés des deux ND | `[SIM]` |
| 2.3 | MCDU | accès au FMGC | `<FMGC` depuis `MCDU MENU` | la page racine porte `<FMGC` `<ATSU` `<AIDS` `<CFDS` — **le FMGC ne s'ouvre pas seul, il se sélectionne** (§2.3) | `[WIP]` |
...
| 4.1 | ELECTRICAL | `EXT PWR` | OFF | `APU GEN` déjà en ligne (3.3) : la bascule se fait sans coupure, mémo `EXT PWR` disparu. Couper l'externe avant, c'est retomber sur batteries | `[WIP]` `[RÉEL]` |
...
| 9.5 | hors Systems | mémos sol et `FOB` | contrôle | `EXT PWR` et `DOORS OPEN` tombés, `FOB` lu à l'E/WD et cohérent avec le bloc de l'OFP. Le contrôle `GW − FOB` se fait ici (§7 · AVANT MISE EN ROUTE) | `[WIP]` |
...
Deux captures relevées. Ce qu'elles montrent est ailleurs que là où le dossier attendait.

| Point | Attendu du live | Réellement fermé ? |
|---|---|---|
| Rangée basse des quatre panneaux | oui, si montrés entiers | **non** — aucun panneau Systems dans les captures |
| Emplacement de la commande `ADIRS` | oui | **non** — l'overhead n'est pas dans le champ |
| Positions du sélecteur `NORM` | oui | **non** |
| Contenu de `GROUND SERVICES` | oui | **non** — le bouton est visible, le panneau n'est pas ouvert |
| Chaîne systèmes réelle ou simple affichage | non, aucune image ne le montre | **non**, comme prévu |

⇒ Ce que les captures ont ouvert à la place : le menu MISC (§7.0 bis), le mémo d'alignement ADIRS (§7), la page racine `MCDU MENU` (§2.3), l'approche automatique (§6.7).
⚠ **Un live ne lève aucun `[WIP]`** (§A.3 règle 12). Il remplace du matériel studio incomplet par du matériel studio complet : ce sont les `[MANQUE]` qu'il purge — quand il les vise. Les cinq points ci-dessus restent donc ouverts, et le sont depuis la REV 20 pour le dernier.
...
⚠ **Ce menu est propre au simulateur, pas à l’appareil.** Il ne se duplique pas par type et ne dépend d’aucune condition d’entrée : ce qui suit vaut sur A320 comme sur 777.

Relevé sur capture studio du 29/08/2026, appareil au parking. Vingt entrées, quatre par rangée. `[WIP]` en entier.

| | | | |
|---|---|---|---|
| AUTO START | TIME x1 | FLIGHT PLAN | ✈ |
| FMS | CHECKLIST | AUTO BRAKES OFF | DEV |
| BRK FAN | ECAM PAGE | CAMERA SNAPSHOT | GROUND SERVICES |
| SNAPSHOTS | MANAGE ORGS | ATC SCHEDULE | SYSTEMS |
| AUTO PAUSE | SAVE | SCENARIO EDITOR | AP OFF |
| — | — | RUD BRAKES | — |

**LES TROIS QUI TOUCHENT LE DOSSIER**

| Entrée | Ce qu'elle fait au dossier | Décision |
|---|---|---|
| `AUTO START` | court-circuite la séquence du §7.0 d’un appui, et les 45 lignes du §7.0.A quand il s’applique | **À ne pas utiliser avant que la séquence manuelle ait été tenue une fois de bout en bout.** Sinon les lignes `[WIP]` ne passeront jamais à `[SIM]` : elles resteront invérifiées parce qu'aucune n'aura été touchée |
| `CHECKLIST` | checklists embarquées, périmètre inconnu | Point ouvert du §8. Si elles couvrent le cold & dark, la règle de non-répétition s’applique et le §7.0 comme le §7.0.A se réduisent à ce qu’elles ne disent pas. Le bouton ne suffit pas : son contenu se lit avant de retirer quoi que ce soit |
| `TIME x1` | accélération temporelle | Un vol accéléré ne remplit pas le §9 : ni écart carburant, ni ETE, ni TOD. Le noter avant de partir, pas après |

**LES AUTRES**

`ECAM PAGE` sélectionne une page SD directement — les attendus SD des phases 3 et 7 (§7.0) restent valables, le chemin change. `AUTO BRAKES` double la commande d'autobrake du build 18266 (§4). `BRK FAN`, `SNAPSHOTS`, `CAMERA SNAPSHOT`, `MANAGE ORGS`, `ATC SCHEDULE`, `SAVE`, `AUTO PAUSE` et `SCENARIO EDITOR` sont hors périmètre du briefing.

⚠ `DEV` est présent sur cette capture comme sur celles des quatre panneaux Systems. Le menu montré est donc un build de développement : la disposition livrée peut différer, et une entrée peut disparaître d'ici la release.
...
⇒ Ce n'est pas un bug, c'est la procédure. Une capture studio annonce désormais un mémo `IR IN ALIGN 2 MN` à l'E/WD (voir plus bas) ; tant qu'il n'a pas été vu en session, la règle reste celle de la REV 21 — aucun message ne dit que l'alignement manque, l'écran est simplement vide.
...
**CE QUI A CHANGÉ EN REV 24.** La REV 21 écrivait que rien ne rappelle l'alignement manquant et que l'écran est simplement vide. Une capture studio du 29/08/2026 montre un mémo `IR IN ALIGN 2 MN` à l'E/WD, avec le temps restant.
⇒ Si ce mémo est bien dans le build, l'alignement se surveille au décompte et non à l'œil, et la ligne « mémo d'alignement ADIRS » du §2.3 tient sa confirmation.
⚠ `[WIP]` : matière studio, sur un appareil non identifié. **La phrase de la REV 21 reste vraie jusqu'à ce que le mémo soit vu en session** — c'est elle qui décrit ce que Milan a observé, et une image ne remplace pas une observation (§A.3 règle 12). La ligne du §2.3 ne sort donc pas du tableau « à revérifier ».
...
**CONTRÔLE DE MASSE AU POSTE**
`GW` en bas de page SD − `FOB` à l'E/WD = masse sans carburant réelle, à comparer à l'EZFW de l'OFP.
⇒ C'est le contrôle qui manquait : l'écart de masse du §2.3 se voyait après le vol, il se voit maintenant avant le repoussage.
⚠ Écart supérieur à une tonne : les vitesses de l'OFP ont été calculées sur une autre masse. Reprendre §2.2 ②, et vérifier la marge au MTOW sur la masse lue, pas sur celle du plan.
⚠ `[WIP]` sur les valeurs de la capture (`GW 79600` pour `FOB 3920`), `[SIM]` dès que la lecture est faite une fois dans le build. La méthode, elle, ne dépend d'aucune capture : les deux champs existent sur tous les Airbus d'IF.

⇒ Sur un type qui impose une séquence de mise en route, ce bloc vient **après** le §7.0, pas avant : on ne configure pas un avion qui n'est pas alimenté.
...
- **Mémos `IR IN ALIGN`, `EXT PWR` et `DOORS OPEN`** au sol : états normaux eux aussi. Ils décrivent la situation, ils ne la reprochent pas — et ils tombent d'eux-mêmes quand la situation change. `[WIP]`, relevés sur capture studio du 29/08/2026.
...
⚠ **Ni le §7.0 ni le §7.0.A ne sont bloquants, et ne le seront jamais.** Une séquence de mise en route mal comprise se voit au parking, avant l’EOBT, et ne fait décoller personne avec un chiffre faux. C’est une réserve de bloc 5, pas un point d’arrêt.

**À VÉRIFIER, non bloquant** :
OFP de moins de 2 h · MIN DIV et BINGO calculés · distance MCDU = distance OFP · briefing panne fait · schedule volets sorti pour le type · **39 cellules des cartouches renseignées** · **neuf champs du §0.2 bis renseignés, aucun laissé à sa valeur de démonstration** · type et masse à vide donnés par Milan, aucune alerte du §A.2 restée sans réponse · **ADIRS aligné, PFD et ND vivants** · **séquence du §7.0 tenue si le type en impose une, §7.0.A tenu si `Type` vaut `A21N`** · **`AUTO START` non utilisé tant que la séquence n'a pas été tenue une fois** · **mémo T.O tout vert** · **`GW − FOB` comparé à l'EZFW** · Aircraft Health regardé, relevé postérieur au 21/08 · build inchangé depuis la dernière observation du FMS · contrôle §A.5 passé si la feuille part en fichier
...
| §7.0.A — rangée basse coupée sur les quatre panneaux | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 20 |
| §7.0.A — emplacement de la commande `ADIRS` | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 21 |
| §7.0.A — positions réelles du sélecteur `NORM` | **inventaire** : une capture studio qui les vise, et tombe avec la rangée basse si le panneau ENGINE est montré entier | REV 21 |
| §7.0.A — contenu du panneau `GROUND SERVICES` | **inventaire** : une capture studio qui les vise. Le live du 28/08 ne les a pas visés | REV 21 |
| §7.0.A — chaîne systèmes réellement simulée, ou état d'affichage | **comportement** : le test discriminant du §7.0.A, deux minutes au parking. Aucune image ne le ferme | REV 20 |
| **Périmètre de la checklist embarquée** (§7.0 bis) | ouvrir l'entrée `CHECKLIST` et lire ce qu'elle porte. Tant que ce n'est pas fait, aucune ligne du §7.0 ni du §7.0.A ne se retire au motif qu'elle ferait doublon | REV 24 |
| **Mémo `IR IN ALIGN` dans le build** (§7 · ADIRS) | une mise sous tension, ADIRS non aligné, regarder l'E/WD | REV 24 |
| **Contrôle `GW − FOB` au poste** (§7) | une lecture au parking, comparée à l'EZFW de l'OFP | REV 24 |
| **Approche automatique CAT3 DUAL** (§6.7) | un vol : le FMA l'affiche ou il ne l'affiche pas | REV 24 |
| **Registre des types du §7.0 — lignes Boeing, Embraer et autres en `[MANQUE]`** | trois observations au parking par type : séquence requise, alignement bloquant, pages SD vivantes. Aucun vol nécessaire | REV 25 |
| **Genre des 39 clés du §A.5 — répartition reconstruite, non lue** | une lecture de `js/data/fields.js`, clé par clé. Sans voler, sans capture. La même lecture ferme le point de la parenthèse de traduction | REV 29 |
| **Atomicité — deux nombres séparés par un blanc sont soudés et acceptés** (§A.5 règle 1) | correctif de code : tester l'atomicité avant d'écraser les blancs. **Défaut silencieux** : aucune feuille n'est protégée en attendant, sauf `Altitude initiale` | REV 30 |
| **Borne du cap contournée par la prose** (§A.5 genre `cap`) | correctif de code : le balayage de prose ne réécrit jamais une clé que la validation a refusée. **Défaut silencieux** | REV 30 |
| **Marqueurs — casse, crochet fermant, marqueur sur libellé** (§A.5) | correctif de code, trois retouches d'expression régulière. Le troisième cas est silencieux | REV 30 |
| **`Altitude (Pieds)` rendue en niveau de vol** (§A.5) | décision, pas observation : ce rendu est voulu ou il ne l'est pas. S'il l'est, il s'écrit comme tel ; sinon il se retire | REV 30 |

⚠ **Les cinq points §7.0.A ne bloquent aucun vol.** L'A321neo n'est pas au sélecteur de Milan : la section ne s'applique à rien de volable aujourd'hui, et l'ordre qu'elle pose ne dépend d'aucune de ces réponses. Ils se ferment le jour où le type entre au sélecteur — pas avant, et sans rien retarder.
⇒ **Les quatre premiers sont des questions d'inventaire** : ils demandent de voir le menu entier, pas de le manipuler. Le live du studio du **28/08/2026** devait être cette occasion : il ne l'a pas été — aucun panneau Systems n'y figure (§7.0.A, « ce que le live a fermé »). Ils attendent une capture qui les vise, ou le menu à l'écran. Le cinquième est une question de comportement : il ne se ferme qu'aux commandes.
⚠ **Le §6.7 ne bloque rien non plus.** Une approche automatique qui n'existe pas dans le build ne bloque aucun départ : elle ne se briefe pas, voilà tout.
⚠ **Trois des quatre points ajoutés en REV 24 se ferment sans voler** : une mise sous tension au parking suffit pour le mémo ADIRS, la lecture `GW − FOB` et le contenu de la checklist embarquée. Ce sont les moins chers du tableau.
⚠ Fermer les quatre premiers **ne lève aucun `[WIP]`** du §7.0.A (§A.3 règle 12) : un live complète le matériel studio, il ne le vérifie pas.
...
⚠ **Un vol conduit en accélération temporelle (§7.0 bis) ne remplit pas cette section.** Ni carburant, ni ETE, ni TOD, ni Fuel Factor. Le noter avant de partir : c'est un vol qui ne compte pas pour le dossier.
...
**ÉCART DE MASSE — RELEVÉ AU POSTE**
`GW` lu ____ · `FOB` lu ____ · masse sans carburant obtenue ____ · EZFW de l'OFP ____ · écart ____
⇒ Trois relevés suffisent à dire si l'écart du §2.3 est constant ou proportionnel à la masse. C'est la différence entre un décalage qu'on compense et un défaut qu'on subit.
**ÉCARTS EN VOL**
...
⇒ Un atterrissage automatique, le jour où le §6.7 se réveille, se relève ici comme un autre — et se compare à un manuel à masse comparable.
...
**SYSTÈMES — SI LE TYPE VOLÉ EN IMPOSE UNE SÉQUENCE (§7.0), ET S’IL A UN BLOC SPÉCIFIQUE (§7.0.A)**
...
Mémo `IR IN ALIGN` vu ou absent ____ ⇒ ferme le point ouvert du §8 et la ligne ADIRS du §2.3.
Contenu de l'entrée `CHECKLIST` ____ ⇒ décide si le §7.0 et le §7.0.A se réduisent, et de combien.
Page racine `MCDU MENU` : quatre entrées confirmées ? ____ ⇒ ferme la ligne `[WIP]` du §2.3.
⇒ Chaque ligne remplie fait passer une ligne du §7.0.A de `[WIP]` à `[SIM]`, ou la supprime. Les trois premières ferment un point ouvert du §8.

**Type volé absent du registre du §7.0** : ____ · séquence requise ? ____ · alignement ADIRS bloquant ? ____ · pages SD vivantes ? ____
⇒ Ces trois réponses créent la ligne du registre. Elles se lisent au parking, sans voler.

**FIN · REV 30 · Simulation uniquement**
